#!/usr/bin/env python3
"""
Smoke test for Parler TTS integration with OpenAI-compatible TTS API.
This test attempts to connect to the API (started independently) and generate speech using Parler TTS.
"""
...
def test_parler_tts_api():
    """Test the OpenAI-compatible TTS API with Parler TTS model."""
    api_url = "http://127.0.0.1:7778/v1/audio/speech"

    # Test data for Parler TTS
    test_request = {
        "model": "parler-tts",
        "input": "Hello, this is a test of the Parler TTS integration with the OpenAI-compatible API.",
        "voice": "default",
        "response_format": "wav",
        "params": {
    ...
    try:
        print("Testing Parler TTS API endpoint...")
        ...
        response = requests.post(
            ...
            timeout=120  # Parler TTS might take longer to load and generate
        )

        print(f"✓ API call completed with status code: {response.status_code}")

        if response.status_code == 200:
            print("✓ Parler TTS API call successful!")
        ...
        print("✗ API request timed out. The Parler TTS model might be taking too long to load or generate audio.")
        return False
...
if __name__ == "__main__":
    ...
    # Run the test
    success = test_parler_tts_api()

    print("\n" + "=" * 50)
    if success:
        print("✓ Smoke test PASSED - Parler TTS integration is working!")
        sys.exit(0)
    else:
